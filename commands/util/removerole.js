module.exports = {
  ...
  async execute(message, args, cmd, client, Discord, profileData) {
    ...
    try {
      await mentionedMemberId.roles.remove(role);
      message.channel.send(`Succesfully removed the @${role} from ${mentionedMember.user.username}`);
    } catch (err) {
      console.log(err);
      message.channel.send("An error occured when removing the role to the member! :x:");
    }
  }
}